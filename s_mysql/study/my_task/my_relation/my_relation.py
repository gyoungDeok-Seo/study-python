from my_crud_module import *
from my_field_trip import FieldTrip
from my_post import Post
from my_owner import Owner
from my_conference_room import ConferenceRoom
import hashlib
...
if __name__ == '__main__':
    ...
    # password1.update('1234'.encode('utf-8'))
    # password1 = password1.hexdigest()
    #
    # password2 = hashlib.sha256()
    # password2.update('5678'.encode('utf-8'))
    # password2 = password2.hexdigest()
    #
    # password3 = hashlib.sha256()
    # password3.update('9012'.encode('utf-8'))
    # password3 = password3.hexdigest()
    #
    # save_many_params = (
    #     ('hgd1234@gmail.com', password1, '홍길동'),
    #     ('kcs5678@naver.com', password2, '김철수'),
    #     ('kyh9012@daum.net', password3, '김영희'),
    # )
    #
    # save_many(save_many_query, save_many_params)
    ...
    # find_by_id_params = 1,
    #
    # office_id = find_by_id(find_by_id_query, find_by_id_params)
    #
    ...
    # 회의실마다 이용가능 시간 추가
    # find_all_query = "select id from tbl_conference_room"
    # conference_room_ids_dict = find_all(find_all_query)
    #
    # save_query = "insert into tbl_part_time(time, conference_room_id) \
    #               values(%s, %s)"
    # for conference_room_id in conference_room_ids_dict:
    #     for i in range(24):
    #         if i < 10:
    #             save_params = f"0{i}:00:00", conference_room_id.get('id')
    #             save(save_query, save_params)
    #         else:
    #             save_params = f"{i}:00:00", conference_room_id.get('id')
    #             save(save_query, save_params)

    # 예약 추가
    # 예약할 회의실의 번호를 가져온다.
    # find_by_id_query = "select id from tbl_conference_room \
    #                     where id = %s"
    # find_by_id_params = 5,
    # conference_room = find_by_id(find_by_id_query, find_by_id_params)
    # conference_room_id = conference_room.get('id')
    #
    # # 회의실 번호와 시간대로 해당 파트타임의 번호를 가져온다.
    # find_by_id_query = "select id from tbl_part_time \
    #                     where conference_room_id = %s and time = %s"
    # reservation_time = '14:00:00'
    # find_by_id_params = conference_room_id, reservation_time
    # part_time = find_by_id(find_by_id_query, find_by_id_params)
    # part_time_id = part_time.get('id')
    #
    # # 파트타임 번호로 해당 파트타임의 status의 데이터를 '예약 완료'로 수정한다.
    # update_query = "update tbl_part_time set status = '예약 완료' \
    #                 where id = %s"
    # update_params = part_time_id,
    # update(update_query, update_params)
    #
    # # 예약 테이블에 데이터를 저장 위해 클라이언트의 이메일을 가져온다.
    # find_by_id_query = "select email from tbl_client \
    #                     where email = %s"
    # # find_by_id_params = 'hgd1234@gmail.com',
    # # find_by_id_params = 'kcs5678@naver.com'
    # find_by_id_params = 'kyh9012@daum.net'
    # client = find_by_id(find_by_id_query, find_by_id_params)
    # client_email = client.get('email')
    #
    # # 가져온 시간대, 클라이언트 이메일, 회의실 번호로 예약 테이블에 데이터를 저장한다.
    # save_query = "insert into tbl_reservation(time, client_email, conference_room_id) \
    #               values(%s, %s, %s)"
    # save_params = reservation_time, client_email, conference_room_id
    # save(save_query, save_params)

    # 회의실 전체 내용 조회(예약이 이미 완료된 회의실 시간은 보여지지 않는다).
    # 조회할 회의실의 번호를 가져온다.
    find_by_id_query = "select id from tbl_office \
                        where id = %s"
    find_by_id_params = 1,
    office = find_by_id(find_by_id_query, find_by_id_params)
    office_id = office.get('id')

    find_all_by_query = "select cr.id, cr.office_id \
                             from tbl_office o \
                             join tbl_conference_room cr \
                             on o.id = %s and o.id = cr.office_id"
    find_all_by_params = office_id,
    conference_rooms = find_all_by(find_all_by_query, find_all_by_params)

    find_all_by_query = "select pt.id, pt.time, pt.status \
                         from tbl_conference_room cr \
                         join tbl_part_time pt \
                         on cr.office_id = %s and cr.id = %s \
                         and cr.id = pt.conference_room_id"

    conference_room_list = []
    for conference_room in conference_rooms:
        find_all_by_params = office_id, conference_room.get('id')
        part_times = find_all_by(find_all_by_query, find_all_by_params)
        conference_room = ConferenceRoom(conference_room.get('id'), conference_room.get('office_id'), part_times)
        conference_room_list.append(conference_room.__dict__)

    for conference_room_info in conference_room_list:
        part_times_info = conference_room_info.get('part_times')
        count = 0
        del_list = []
        for part_time_info in part_times_info:
            part_time_info['time'] = str(part_time_info.get('time'))
            if part_time_info.get('status') == '예약 완료':
                count += 1
                del_list.append(part_time_info)

        for i in range(count):
            part_times_info.remove(del_list[i])

        print(conference_room_info)
