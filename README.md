...
#### 인터프리터  
    ...
    인터프리터는 매번 소스코드를 한 줄 씩 해석해서 실핼하므로(개별처리),
    전체 프로그램의 포퍼먼스에 큰 손해를 본다.
    파이썬은 소스코드를 바이트 모드로 컴파일한 뒤
...
#### 알고리즘과 자료구조
    ...
    ※ 링크드리스트를 매개변수 형변환을 어레이 리스트로확용 속도가 빠르기 때문에
...
#### 연산자 - 기능이 있는 특수문자
    ...
    6. 식별 연산자 * 참조 - 주소값에 담겨있는 값을 읽어오는 행위
...
#### 반복문
    for: in절 뒤의 요소를 순서대로 변수에 담고 값이 더 이상 없을 경우 종료
...
#### list 선언
    ...
    - range로 만들 경우 타입은 range가 되기 때문에 list()로 형변환해준다.
...
#### kwargs 함수 사용 방법
    1. 여러 개의 값을 콤마로 구분하여, ket=value 형태로 전달
...
#### 클로저(closure, 폐쇄): 함수 안에 함수, 모듈화
    ...
    함수가 종료된 이후에도 지역변수에 접근할 수 있기 때문에 데이터 지속정을 가지고 있다.
...
#### super().__init__(): 부모 생성자
    자식 객체로 타입의 객체로 부모 필도에 접근할 수 있다.
...
#### 오버라이딩(재정의, 무시하기)
    부모필드에서 선언한 메소드를 자식 필드에서 수정하고자 할 떄 재정의를 해야한다.
    이는 자식에서 부모 필드의 메소드와 동일한 이름으로 선언하는 문법을 의미한다.
    접근한 객체와 가까운 곳부터 찾기 때문에, 자식 필드에 해당 메소드가 있다면 재정의 메소드가 실행된다.
...
#### 패키지
    폴더를 생성하여 .py 또는 .ipynd파일을 관리하고자 할 때 해당 폴더를 패키지라고 한다.
    __init__.py 파일을 생성해야 패키지로 인식되지만, 상위 버전(3.3부터)에서는
    __init__.py 파일이 없어도 패키지로 인식된다.
    하지만, 하위 버전(3.3 미만)에서도 인식되시 위해서는 직접 생성해 놓는 것이 좋다.
